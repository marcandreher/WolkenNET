...
public class KlinikUndÄrzte implements Route {
	public Map<String, Object> m = new HashMap<>();

	public KlinikUndÄrzte() {
	}

	public Object handle(Request request, Response response) {
		Permissions.hasPermissions(request.cookie("session"), this.m, response);
		m.put("titlebar", "Klinik & Ärzte");
		...
		try {
			...
			ResultSet pages_rs = mysql.Query(pages, 3 + "");
			while (pages_rs.next()) {
				pagesInt++;
			}
			...
			ResultSet tags_rs = mysql.Query(tags_sql, 3 + "");
			while (tags_rs.next()) {
		...
		try {
			Template template = App.cfg.getTemplate("klinik.html");
